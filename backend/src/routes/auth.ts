...
import { authRequired, requireRole, type AuthRequest } from '../middleware/auth.js';
import { createPlainToken, hashToken, minutesFromNow } from '../utils/authTokens.js';
import { emailEnabled, sendActivationEmail, sendPasswordResetEmail } from '../services/email.js';
...
authRouter.post('/login', asyncHandler(async (req, res) => {
  ...
  if (user.status === 'PENDING') return res.status(403).json({ error: 'Account not activated', code: 'ACCOUNT_PENDING' });
  if (user.status === 'SUSPENDED') return res.status(403).json({ error: 'Account suspended', code: 'ACCOUNT_SUSPENDED' });
  if (user.status === 'DISABLED') return res.status(403).json({ error: 'Account disabled', code: 'ACCOUNT_DISABLED' });

  const updated = await prisma.user.update({ where: { id: user.id }, data: { lastLoginAt: new Date() } });
  return res.json(authPayload(updated));
}));

authRouter.get('/me', authRequired, asyncHandler(async (req: AuthRequest, res) => {
...
authRouter.post('/admin/activate-user', authRequired, requireRole('ADMIN'), asyncHandler(async (req: AuthRequest, res) => {
